...
4. Incoming control of this Hyprland desktop is **not** solved by the deploy script. See [INCOMING.md](INCOMING.md). If YTOmarchy1 shows **"connection type is not allowed on this machine"**, that is the controller-side message for the same failure: the host handshake succeeds, then `TeamViewer_Desktop` dies (session + missing `RemoteDesktop` portal) and the session times out.
...
## Incoming control (YTOmarchy1 → this PC)

Tracked in [INCOMING.md](INCOMING.md) for the other Omarchy box’s Grok session.

Verified on OmarchyT430s (host ID `1214731265`) while YTOmarchy1 (ID `809843398`) connected:

- Routers and TLS handshake succeed.
- `teamviewerd` then starts `/opt/teamviewer/tv_bin/TeamViewer_Desktop` from `system.slice` → `LaunchDesktopProcess: session bus not found` → `Own session could not be resolved` → process exits.
- Hyprland’s portal implements ScreenCast, not `org.freedesktop.portal.RemoteDesktop`, which TeamViewer’s Wayland path requires.
- Host then logs `AuthenticationTimeout`. YTOmarchy1 surfaces **"connection type is not allowed on this machine"**.

The deploy script does not fix that. Outgoing from this PC can still work.

## Limitations

- TeamViewer 15 on Linux is still an X11/Qt5 client. The deploy script is a working Wayland *client* (outgoing) setup plus daemon/ffmpeg glue, not full native Wayland incoming control.
